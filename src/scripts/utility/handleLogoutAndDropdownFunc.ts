...
const { cartQuantity, signUpBtnElem, logInBtnElem } = homepageElems;
...
export const handleCartIcon = () => {
  let addProductsToCart =
    JSON.parse(localStorage.getItem("addProductsToCart")) || [];
  cartQuantity.innerHTML = addProductsToCart.length;
  addProductsToCart.length === 0 || null
    ? cartQuantity.classList.add("none_elem")
    : cartQuantity.classList.remove("none_elem");
};
